...
import { useBills, useUpsertBill, useDeleteBill, useBillPayments, useTogglePaid, type Bill } from "@/lib/data";
import { fmtMoney, BILL_CATEGORIES } from "@/lib/finance";
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`${fmtMoney(paidTotal)} paid of ${fmtMoney(total)} this month`}
        action={<Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="size-4 mr-1" />Add bill</Button>}
      />
            ...
            {[...data].sort((a, b) => a.due_day - b.due_day).map(b => {
              ...
              return (
                <li key={b.id} className={cn("flex items-center gap-3 p-4 border-b border-border last:border-0", paid && "opacity-60")}>
                  <Checkbox
                    checked={paid}
                    onCheckedChange={v => toggle.mutate({ billId: b.id, periodMonth, paid: !!v })}
                  />
